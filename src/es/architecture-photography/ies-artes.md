---
title: "IES Artés"
permalink: "{{ preSlug  }}/emba-ies-artes/"
description: "Instituto en Artés por EMBA, Enric Massip-Bosch Architects. Fotografía de Arquitectura por Pau Guerrero."
date: "2004-02-10"
location: "Artés, Barcelona"
architect: Estudi Massip-Bosch Architects
teaser: "images/teaser-emba-ies-artes.jpg"
teaserAlt: "Diseños circulares coloridos en el techo proyectan sombras en un pasillo cubierto sostenido por columnas de hormigón."
images:
  - layout: vh
    items: 
      - src: "images/emba-ies-artes-01.jpg"
        alt: "Diseños circulares coloridos en el techo proyectan sombras en un pasillo cubierto sostenido por columnas de hormigón."
        caption: ""
        contentLocation: "Artés, Barcelona"
        name: "IES Artés - Estudi Massip-Bosch Arquitectes"
      - src: "images/emba-ies-artes-04.jpg"
        alt: "Vista exterior con una fachada con lamas metálicas verticales y columnas de hormigón que sostienen la estructura."
        caption: ""
        contentLocation: "Artés, Barcelona"
        name: "IES Artés - Estudi Massip-Bosch Arquitectes"
  - layout: hh
    items:         
      - src: "images/emba-ies-artes-07.jpg"
        alt: "Vista angular de la fachada del Instituto Artes mostrando las lamas metálicas verticales contra un cielo azul despejado, enfatizando el diseño arquitectónico moderno."
        caption: ""
        contentLocation: "Artés, Barcelona"
        name: "IES Artés - Estudi Massip-Bosch Arquitectes"
      - src: "images/emba-ies-artes-02.jpg"
        alt: "Vista exterior con un diseño moderno con revestimiento metálico vertical y una valla de seguridad a lo largo del perímetro."
        caption: ""
        contentLocation: "Artés, Barcelona"
        name: "IES Artés - Estudi Massip-Bosch Arquitectes"
  - layout: hh
    items:         
      - src: "images/emba-ies-artes-03.jpg"
        alt: "Vista interior de un gimnasio con vigas y paredes de hormigón expuesto, grandes ventanas y equipo de gimnasia de madera a lo largo de una pared."
        caption: ""
        contentLocation: "Artés, Barcelona"
        name: "IES Artés - Estudi Massip-Bosch Arquitectes"
      - src: "images/emba-ies-artes-06.jpg"
        alt: "Vista de cerca de las lamas metálicas verticales que forman la fachada del Instituto Artes, creando un patrón dinámico y texturizado."
        caption: ""
        contentLocation: "Artés, Barcelona"
        name: "IES Artés - Estudi Massip-Bosch Arquitectes"
  - layout: hh
    items:         
      - src: "images/emba-ies-artes-05.jpg"
        alt: "Vista exterior que muestra una espaciosa pista deportiva al aire libre y la fachada del edificio con lamas metálicas verticales y columnas de hormigón."
        caption: ""
        contentLocation: "Artés, Barcelona"
        name: "IES Artés - Estudi Massip-Bosch Arquitectes"
      - src: "images/emba-ies-artes-08.jpg"
        alt: "Fotografía arquitectónica en ángulo bajo de un pabellón exterior cubierto. La perspectiva sigue una hilera de pilares de hormigón visto que proyectan sombras marcadas sobre el suelo. El techo de hormigón destaca por un diseño geométrico de grandes círculos de colores vibrantes en tonos amarillo, naranja y rojo. Al fondo, unas gradas de hormigón conducen a una pista deportiva exterior delimitada por una valla metálica."
        caption: ""
        contentLocation: "Artés, Barcelona"
        name: "IES Artés - Estudi Massip-Bosch Arquitectes"
---

## Fotografía de arquitectura educativa: IES Artés por EMBA

Esta colección de fotografía arquitectónica institucional documenta la moderna ampliación y estructura del **IES Artés**, una escuela secundaria pública en **Artés, Barcelona**. Diseñado por **[EMBA, Estudi Massip-Bosch Arquitectes](https://www.emba.cat)** bajo la dirección de Enric Massip-Bosch, este edificio educativo ofrece una visión sofisticada del diseño institucional cívico, optimizando los materiales estructurales en bruto para crear un entorno de aprendizaje dinámico y luminoso.

La sesión fotográfica llama la atención sobre los patrones lineales nítidos, el juego dinámico de sombras y la honestidad material que definen este campus contemporáneo.

### Capturando lamas exteriores, columnas de hormigón y sombras circulares

La fotografía de arquitectura de exteriores se centra en el ritmo estructural, el encuadre geométrico y el uso de la luz solar natural por parte del edificio.

* **Lamas verticales y patrones lineales:** Una característica definitoria de la fachada de la escuela es el uso de un elegante **revestimiento metálico y lamas verticales**. La fotografía utiliza encuadres precisos de gran angular para capturar estas líneas repetitivas, mostrando cómo proporcionan privacidad y protección solar al tiempo que dotan al volumen institucional de una identidad limpia y unificada.
* **Juego lúdico de sombras geométricas:** Una secuencia destacada de esta serie arquitectónica se centra en los pasillos exteriores cubiertos del edificio. Sostenidas por columnas de hormigón, estas estructuras superiores presentan atrevidos y coloridos **diseños circulares en el techo**. La fotografía captura el sol mediterráneo pasando a través de estos recortes geométricos, proyectando patrones dinámicos y giratorios de luz y sombra a lo largo de los caminos de hormigón inferiores.

### Documentando espacios luminosos y materialidad interior en bruto

Fotografiar los espacios interiores del IES Artés requiere capturar la permanencia estructural junto con una distribución diáfana y abierta adecuada para el movimiento de los estudiantes.

La fotografía muestra el enorme gimnasio interior de la escuela, que exhibe una paleta de inspiración brutalista de **vigas y muros de hormigón visto**. Las composiciones enmarcan los imponentes volúmenes de doble altura, mostrando cómo las elevadas cuadrículas de los grandes ventanales llenan las superficies de hormigón en bruto con una luz diurna suave y uniforme. Cada encuadre de esta galería institucional mantiene una estricta **alineación ortogonal**, manteniendo los altos pilares, las vallas metálicas y las juntas de las ventanas perfectamente verticales. Esta colección proporciona un archivo visual duradero y de alta fidelidad de la contribución de EMBA a las obras públicas y al diseño educativo progresivo en Cataluña.