---
title: The Other Art Fair, Sydney
permalink: "{{ preSlug  }}/the-other-art-fair-sydney/"
description: Exposició "The Print Swap" a la fira d'art The Other Art Fair, a Sydney, Austràlia.
intro: "Exposició Col·lectiva <em>The Print Swap</em> by Feature Shoot a la Fira d'Art <a class='is-intro-link' href='https://www.theotherartfair.com/'>The Other Art Fair</a>, Sydney"
date: 2019-03-17
teaser: images/teaser-pau-guerrero-print-swap.jpg
teaserAlt: "Una exposició d'art plena de gent amb moltes persones veient diverses obres d'art en un espai ampli d'estil industrial amb sostres alts i bigues exposades."
location: Sydney, Australia
venue: The other Art Fair
images:
  - layout: hh
    items: 
      - src: images/pau-guerrero-print-swap-01.jpg
        alt: "Tres persones veient fotografies emmarcades exposades a l'exposició 'The Print Swap by Feature Shoot'."
        contentLocation: "Sydney, Australia"
        name: "The Other Art Fair"
        description: ""
      - src: images/pau-guerrero-print-swap-02.jpg
        alt: "Una exposició d'art plena de gent amb moltes persones veient diverses obres d'art en un espai ampli d'estil industrial amb sostres alts i bigues exposades."
        contentLocation: "Sydney, Australia"
        name: "The Other Art Fair"
        description: ""
---

_The Print Swap_ va presentar 21 obres de diferents fotògrafs d'arreu del món durant _The Other Art Fair_ a l'Australian Technology Park de Sydney.
